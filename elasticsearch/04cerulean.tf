resource "aws_instance" "elasticsearch-cerulean" {
  connection {
    user = "ubuntu"
    key_file = "${var.key_path}"
  }
  tags {
    Name = "elasticsearch-cerulean"
    Elasticsearch = "true"
  }
...
#change hostname
  ...
  provisioner "remote-exec" {
    inline = [
        "sudo sh /home/ubuntu/change-hostname.sh elasticsearch-cerulean",
...
#install and configure elasticsearch
  ...
  provisioner "file" {
        source = "scripts/elasticsearch-cerulean.yml"
